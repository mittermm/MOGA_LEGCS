...
def log_pareto_front(generation, pareto_front):
    ...
    for s in pareto_front:
        print("\t", s.simID, " -> ", s.objectives)
        logging.info(s.simID + " -> " + str(s.objectives))
        log_full_infos.append([s.simID, s.variables, s.objectives])
        log_infos.append(s.objectives)
    log_full_infos.append([])

    with open("output/pareto_fronts_with_graphs.csv", 'a') as file:
        writer = csv.writer(file)
        writer.writerows(log_full_infos)
    
    with open("output/pareto_fronts", 'a') as file:
        writer = csv.writer(file)
...
class Emb_LEGCS_Evaluator(Evaluator[S]):
    ...
    def evaluate(self, solution_list: List[S], problem: Problem) -> List[S]:
        ...
        if self.generation == 0:
            for s in solution_list:
                simulated_solutions.append(s)
                self.train_sim.append(s.objectives[0])
            ...
            for solution in solution_list:
                ...
                solution.objectives[0] = predictions[genome_counter]
                solution.objectives[1] = get_street_length(solution.variables)
                genome_counter += 1
            ...
            for solution in solution_list:
                if solution.simID in sim_list:
                    solution.objectives[0] = get_travel_time(solution.simID, solution.variables)                
                    self.train_emb.append(embedding[genome_counter])
                    self.train_sim.append(solution.objectives[0])
                    simulated_solutions.append(solution)
                    logging.info(solution.simID + ": fitness = " + str(solution.objectives))
                genome_counter += 1
        
        pareto_front = get_non_dominated_solutions(simulated_solutions)
...
class LEGCS_Evaluator(Evaluator[S]):
    ...
    def evaluate(self, solution_list: List[S], problem: Problem) -> List[S]:
        ...
        if self.generation == 0:
            for solution in solution_list:
                simulated_solutions.append(solution)
                self.dataset.append(get_graph_data(solution.variables), solution.objectives[0])
            self.train_gnn()
        else:
            genome_counter = 0
            for solution in solution_list:
                ...
                solution.simID = "gen" + str(self.generation) + "genome" + str(genome_counter)
                self.model.eval()
                solution.objectives[0] = self.model(get_graph_data(solution.variables)).item()
                solution.objectives[1] = get_street_length(solution.variables)
                genome_counter += 1
    ...
    def evaluate(self, solution_list: List[S], problem: Problem) -> List[S]:        
        if self.generation > 0:
            ...
            for solution in solution_list:
                ...
                solution.simID = "gen" + str(self.generation) + "genome" + str(genome_counter)
                solution.objectives[0] = get_travel_time(solution.simID, genome)
                solution.objectives[1] = get_street_length(genome)
                genome_counter += 1
                logging.info(solution.simID + ": fitness = " + str(solution.objectives))
...
class Traffic_Network_Problem(IntegerProblem):
    def __init__(self):
        super().__init__()
        self.number_of_variables = 32**2  
        self.number_of_objectives = 2  # Two objectives to minimize
    ...
    def name(self):
        return "Traffic_Network_Problem"
...
class TNP_NSGAII(NSGAII):
    def create_initial_solutions(self):
        ...
        for i in range(population_size):
            new_solution = IntegerSolution([0] * num_variables, [5] * num_variables, 2, 0)
            new_solution.variables = initial_population[i]
            new_solution.objectives[0] = sims[i]
            new_solution.objectives[1] = get_street_length(initial_population[i])
...
while iterator < runs:
    ...
    try:
        ...
        problem = Traffic_Network_Problem()

        evaluator = Emb_LEGCS_Evaluator()
        

        # Configure NSGA-II Algorithm
        algorithm = TNP_NSGAII(
            problem=problem,
        ...
        logging.info("starting NSGA-II with " + type(evaluator).__name__ + " with population size = " + str(population_size) + " and offset = " + str(offset))
        ...
        for solution in non_dominated_solutions:
            print(sum(solution.variables), "->", solution.objectives)
